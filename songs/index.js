import she from './she'
import money from './money'
import taxman from './taxman'
import badLiar from './bad-liar'
import ironMan from './iron-man'
import hysteria from './hysteria'
import theChain from './the-chain'
import cantStop from './cant-stop'
import laGrange from './la-grange'
import praiseYou from './praise-you'
import prettyFly from './pretty-fly'
import doReDoRe from './do-re-do-re'
import doMiDoMi from './do-mi-do-mi'
import asaBranca from './asa-branca'
import prayerInC from './prayer-in-c'
import billieJean from './billie-jean'
import blackNight from './black-night'
import thisOldMan from './this-old-man'
import prettyWoman from './pretty-woman'
import feelGoodInc from './feel-good-inc'
import psychoKiller from './psycho-killer'
import frereJacques from './frere-jacques'
import smalltownBoy from './smalltown-boy'
import kidsWithGuns from './kids-with-guns'
import deckTheHalls from './deck-the-halls'
import comeAsYouAre from './come-as-you-are'
import underPressure from './under-pressure'
import happyBirthday from './happy-birthday'
import risinToTheTop from './risin-to-the-top'
import peterGunnTheme from './peter-gunn-theme'
import gimmeSomeLovin from './gimme-some-lovin'
import threeBlindMice from './three-blind-mice'
import youReallyGotMe from './you-really-got-me'
import itsyBitsySpider from './itsy-bitsy-spider'
import sevenNationArmy from './seven-nation-army'
import smokeOnTheWater from './smoke-on-the-water'
import downOnTheCorner from './down-on-the-corner'
import cantaloupeIsland from './cantaloupe-island'
import dismantlingFrank from './dismantling-frank'
import shutUpAndLetMeGo from './shut-up-and-let-me-go'
import rowRowRowYourBoat from './row-row-row-your-boat'
import sunshineOfYourLove from './sunshine-of-your-love'
import maryHadALittleLamb from './mary-had-a-little-lamb'
import oldMcDonalHadAFarm from './old-mcdonald-had-a-farm'
import anotherOneBitesTheDust from './another-one-bites-the-dust'
import allDayAndAllOfTheNight from './all-day-and-all-of-the-night'
import twinkleTwinkleLittleStar from './twinkle-twinkle-little-star'
import ifYouAreHappyAndYouKnowIt from './if-you-are-happy-and-you-know-it'

export default [
  she,
  money,
  taxman,
  badLiar,
  ironMan,
  hysteria,
  theChain,
  cantStop,
  laGrange,
  doReDoRe,
  doMiDoMi,
  asaBranca,
  prayerInC,
  praiseYou,
  prettyFly,
  billieJean,
  blackNight,
  thisOldMan,
  prettyWoman,
  feelGoodInc,
  psychoKiller,
  frereJacques,
  smalltownBoy,
  kidsWithGuns,
  deckTheHalls,
  comeAsYouAre,
  underPressure,
  happyBirthday,
  risinToTheTop,
  gimmeSomeLovin,
  peterGunnTheme,
  threeBlindMice,
  youReallyGotMe,
  itsyBitsySpider,
  sevenNationArmy,
  smokeOnTheWater,
  downOnTheCorner,
  cantaloupeIsland,
  dismantlingFrank,
  shutUpAndLetMeGo,
  rowRowRowYourBoat,
  sunshineOfYourLove,
  maryHadALittleLamb,
  oldMcDonalHadAFarm,
  anotherOneBitesTheDust,
  allDayAndAllOfTheNight,
  twinkleTwinkleLittleStar,
  ifYouAreHappyAndYouKnowIt,
]
